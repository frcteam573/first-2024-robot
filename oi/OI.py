...
class OI:
	...
	def map_controls():
		...
		Keymap.Drivetrain.DRIVE_ALIGN_NOTE.onTrue(
			ParallelCommandGroup(
				...
				commands.SetShoulderAngleFloor(Robot.appendage)
			)
    ...
    .onTrue(InstantCommand(lambda: Sensors.odometry.vision_estimator.limelights[0].change_pipeline(2)),
		...
		Keymap.Drivetrain.DRIVE_ALIGN_AMP.onTrue(commands.SetShoulderAngleAmp(Robot.appendage))
  
		Keymap.Drivetrain.DRIVE_ALIGN_HUMAN.onTrue(commands.SetShoulderAngleHuman(Robot.appendage))
  
		Keymap.Drivetrain.DRIVE_ALIGN_TRAP.onTrue(commands.SetShoulderAngleTrap(Robot.appendage))